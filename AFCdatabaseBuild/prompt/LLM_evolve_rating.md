# 进化阶段 LLM Prompt（evolve_rating）

用于 AFCdatabaseEvolve/update_case.py（或相关模块）中，对一次技能修复/重建的“重建力度”进行 LLM 辅助评估。

与其他 AFCdatabaseBuild/prompt 一样，该模板集中放在本目录，便于统一管理与调优。
